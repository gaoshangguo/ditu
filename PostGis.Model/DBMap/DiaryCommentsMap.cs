...
namespace PostGis.Model.DBMap {
    ...
    public class DiaryCommentsMap : ClassMapping<DiaryComments> {
        ...
        public DiaryCommentsMap() {
			...
			ManyToOne(x => x., map => map.Columns(new Action<IColumnMapper>[] { x => x.Name("diary_entry_id"), x => x.Name("user_id"), x => x.Name("user_id") }));
			ManyToOne(x => x.DiaryEntryId, map => map.Columns(new Action<IColumnMapper>[] { x => x.Name("diary_entry_id"), x => x.Name("user_id"), x => x.Name("user_id") }));
			ManyToOne(x => x.DiaryEntryId, map => map.Columns(new Action<IColumnMapper>[] { x => x.Name("diary_entry_id"), x => x.Name("user_id"), x => x.Name("user_id") }));
        }
    }
}
