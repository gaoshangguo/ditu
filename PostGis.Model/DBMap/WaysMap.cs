...
namespace PostGis.Model.DBMap {
    ...
    public class WaysMap : ClassMapping<Ways> {
        ...
        public WaysMap() {
			...
			Bag(x => x.WayNodes, colmap =>  { colmap.Key(x => x.Column("")); colmap.Inverse(true); }, map => { map.OneToMany(); });
			Bag(x => x.WayTags, colmap =>  { colmap.Key(x => x.Column("")); colmap.Inverse(true); }, map => { map.OneToMany(); });
        }
    }
}
