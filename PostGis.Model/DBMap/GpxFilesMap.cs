...
namespace PostGis.Model.DBMap {
    ...
    public class GpxFilesMap : ClassMapping<GpxFiles> {
        ...
        public GpxFilesMap() {
			...
			ManyToOne(x => x.UserId, map => map.Columns(new Action<IColumnMapper>[] { x => x.Name("user_id"), x => x.Name("user_id") }));
			Bag(x => x.GpsPoints, colmap =>  { colmap.Key(x => x.Column("")); colmap.Inverse(true); }, map => { map.OneToMany(); });
			Bag(x => x.GpxFileTags, colmap =>  { colmap.Key(x => x.Column("")); colmap.Inverse(true); }, map => { map.OneToMany(); });
        }
    }
}
