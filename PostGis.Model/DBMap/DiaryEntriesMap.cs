...
namespace PostGis.Model.DBMap
{
    public class DiaryEntriesMap : ClassMapping<DiaryEntries>
    {
        public DiaryEntriesMap()
        {
            ...
            ManyToOne(x => x.Languages, map => map.Columns(new Action<IColumnMapper>[] { x => x.Name("language_code") }));
            Bag(x => x.DiaryComments, colmap => { colmap.Key(x => x.Column("diary_entry_id")); colmap.Inverse(true); }, map => { map.OneToMany(); });
        }
    }
}
